...
//		DirectoryPath = "C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\4.combine";
		...
		for (int k = 0; k < list.size(); k++)
		{
			...
			Institution_Information_Homepage_Address = "없음 또는 파일 깨짐";
			...
			{// 라인 없을 때까지 읽음 // 라인의 size 측정 { // 한 줄 단위로 처리할 수 있는 readLine 메소드를 이용해서
				...
				else if( s.contains("2)") && (s.contains("부서:") || s.contains("부서 :")))
				{
...
//					  else addLineIndex_Duty = count + 1; // 없음 X ( 데이터 못 잡는 경우 )
	        	...
	        	// 담당 업무 파싱 예외사항
	        	 else if((s.contains("개발")) && Duty_OR_Research.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)") && s.contains(")")) // 예외 사항 모두 묶음 (개발이 많은 사항 발생)
	        	 {
...
//				  Duty_OR_Research.substring(0, Duty_OR_Research.indexOf("(예")).trim(); }
	        	...
	        	// 검색 경로 파싱
	        	 ...
	        	 //Enter 처리 (안상민 잡기)
					...
					// 다음 줄
	        		 if(Service_Access_Path.contains("없음"))
	        			 Service_Access_Path = s.trim();
	        		 else
	        			 Service_Access_Path += s.trim(); // 문자열 붙이기 *질문! => 예)가 다음에 나와있는 상황?
