...
//					}
				...
				//Institution_Location_OR_UserResidence
				 ...
				 if( (s.contains("지역:") || s.contains("지역 :") ) && Institution_Location_OR_UserResidence.equals("없음") )
	        	 {
	        	 ...
	        	 else if(s.contains("시")&&  Institution_Location_OR_UserResidence.equals("없음")) // 시 구까지만 출력 (나머지 데이터는 버림)
	        	 {
...
//					 addLineIndex_Duty = count + 1;
	        		 ...
	        		 // 위에 데이터가 공백인 경우 없음으로 처리
					  ...
					  if(Duty_OR_Research.equals("")) Duty_OR_Research = "없음"; //
					  else addLineIndex_Duty = count + 1; // 없음
        	...
        	// 엔터가 되어있는 경우 처리 ( : 밑에 데이터 있는 경우)
				...
				/*
				 * else if(addLineIndex_Duty == count && !(s.contains(":"))) { Duty_OR_Research
				 * += s; // 문자열 붙이기 *질문! => 예)가 다음에 나와있는 상황?
				 * 
				 ...
				 */
				 ...
				 //바로 위에 코드 수정한 것
					 ...
					 //다음 즐
					 if(Duty_OR_Research.contains("없음")) Duty_OR_Research = s; // 없음 삭제
					 else Duty_OR_Research += s; 
				 }
...
//					 addLineIndex_Key = count + 1; // 그 줄에 원하는 데이터가 없는 경우
	        		 ...
	        		 // 위에 데이터가 공백인 경우 없음으로 처리
					  ...
					  if(Key_Issuance_Purpose.equals("")) Key_Issuance_Purpose = "없음";
					  else addLineIndex_Key = count + 1; // 없음
...
//				 else if (add)
	        	...
	        	// 엔터가 되어있는 경우 처리
				/*
				 * else { if(addLineIndex_Key == count && !(s.contains(":"))) // 다음 줄
				 * Key_Issuance_Purpose += s; // 문자열 붙이기 *질문! => 예)가 다음에 나와있는 상황?
				 * 
				 ...
				 */
				 ...
				 //바로 위에 코드 수정한 것
					 ...
					 //다음 즐
					 if(Key_Issuance_Purpose.contains("없음"))  Key_Issuance_Purpose = s;
	        	...
	        	// 검색 경로 파싱
	        	 if(s.contains("경로") && Service_Access_Path.equals("없음"))
	        	 {
	        		 ...
	        		 // 공백인 경우 없음으로 처리
	        		 if(Service_Access_Path.equals("")) Service_Access_Path = "없음";
	        		 else addLineIndex_Service = count + 1;
	        	 }
	        	 ...
	        	 //Enter 처리 (안상민 잡기)
					 ...
					 //다음 즐
					 if(Service_Access_Path.contains("없음"))  Service_Access_Path = s;
