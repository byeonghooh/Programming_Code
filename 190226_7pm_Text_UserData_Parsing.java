...
//		String DirectoryPath = "C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\combine\\More1_최근자료추가";
...
//		File file = new File("C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\combine\\More1_최근자료추가\\UserData.csv");
...
//		System.out.println("path.listFile 값 " + fileList);
		...
		for (int k = 0; k < list.size(); k++) {
			...
			while ((s = in2.readLine()) != null) {// 라인 없을 때까지 읽음 // 라인의 size 측정 { // 한 줄 단위로 처리할 수 있는 readLine 메소드를
				...
				/*
				 * // User_Name if (s.contains("키발급신청")) { User_Name_Value = s.split("\\]");
				 * System.out.println("내용 있다" + User_Name_Value[1].substring(1, 4)); User_Name =
				 * User_Name_Value[1].substring(1, 4); System.out.println("진짜 칼럼 값" + " : " +
				 * User_Name); if(User_Name.equals("키발급")) {
				 * 
				 * if (s.contains("키발급신청")) {
				 * 
				 * int User_Name_First_idx = s.indexOf("키발급신청"); int User_Name_Last_idx =
				 * s.indexOf("님");
				 * 
...
//					else if(AllText.contains("라고")) {
//						Institution_Last_Idx = s.indexOf("라고");
//						System.out.println("라고 - 잡았나요?");
//					}
...
//					  else addLineIndex_Duty = count + 1; // 없음 X ( 데이터 못 잡는 경우 )
				...
				// Key_Inssuance_Purpose_new
				
				/*
				 * int Key_Issuance_Purpose_First_Idx = AllText.indexOf("목적"); int
				 * Key_Issuance_Purpose_Last_Idx = 0;
				 * 
				 * // 학과가 1번에 위치해 있을 때 - 2번 전에서 끊음 if ( (s.contains("목적:") ||
				 * s.contains("목적 :")) &&
				 * Key_Issuance_Purpose.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)")) {
				 * 
				 * System.out.println(); String subText_Key_Inssuance_Purpose=
				 * AllText.substring(Key_Issuance_Purpose_First_Idx).trim();
				 * 
				 * 
				 * if (subText_Key_Inssuance_Purpose.contains("서비스") &&
				 * (subText_Key_Inssuance_Purpose.contains("4)") ||
				 * subText_Key_Inssuance_Purpose.contains("5)") ) ) {
				 * Key_Issuance_Purpose_Last_Idx = subText_Key_Inssuance_Purpose.indexOf("서비스");
				 * 
				 * System.out.println("서비스의 위치 탐색" + Key_Issuance_Purpose_Last_Idx);
				 * 
				 * Key_Issuance_Purpose = AllText.substring(Key_Issuance_Purpose_First_Idx + 2,
				 * Key_Issuance_Purpose_First_Idx + Key_Issuance_Purpose_Last_Idx).trim();
				 * System.out.println("ID_1!!!!!!!!!!!!!!!!!!!!!! : " + Key_Issuance_Purpose);
				 * 
				 * } else { Key_Issuance_Purpose =
				 * AllText.substring(Key_Issuance_Purpose_First_Idx).trim();
				 * System.out.println("ID_2!!!!!!!!!!!!!!!!!!!!!! : " + Key_Issuance_Purpose);
				 * 
				 * }
				 * 
				 * }
...
//					else {
//						//다음 줄
//						Key_Issuance_Purpose = s.trim(); // 없음 삭제
//					}
//					
//					System.out.println("Key 3 : " + Key_Issuance_Purpose);
//				}
...
//	        			 Service_Access_Path = "없음";
			...
			if( Institution_Location_OR_UserResidence.contains("관리자") && Duty_OR_Research.contains("관리자") && Key_Issuance_Purpose.contains("관리자") && Service_Access_Path.contains("관리자") ) {
				
					Manager_Check_Element = "관리자 확인 필요";
			...
			/*
			 * if(User_Divider.contains("관리자") || Email_Address.contains("관리자") ||
			 * Institution.contains("관리자") || Institution_Department.contains("관리자") ||
			 * Institution_Location_OR_UserResidence.contains("관리자") ||
			 * Duty_OR_Research.contains("관리자") || Key_Issuance_Purpose.contains("관리자") ||
			 * Service_Access_Path.contains("관리자") ) {
			 * 
			 * Manager_Check_Element = "관리자 확인 필요";
			 * 
			 * }
			...
			if (file.isFile() && file.canWrite()) {
				if (first_row_flag == 0) { // 첫째 줄 칼럼명 삽입
					...
					bufferedWriter.write("사용자구분" + "\t" + "이름" + "\t" + "이메일 주소" + "\t" + "소속기관명" + "\t" + "소속부서(학과)명"
							...
							+ "서비스(open API 사이트) 접근 경로" + "\t" + "소속기관정보및홈페이지주소" + "\t" + "관리자 확인 필요성 체크" + "\t" + "메일 전체 내용(관리자 입력 필요 시 활용)" + "\t");
