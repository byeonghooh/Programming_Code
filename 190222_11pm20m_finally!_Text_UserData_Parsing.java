...
//		File file = new File("C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\4.combine\\UserData.csv");
		File file = new File("C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\combine\\More1\\UserData.csv");
...
//		DirectoryPath = "C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\4.combine";
		DirectoryPath = "C:\\Users\\byeonghooh\\eclipse-workspace\\Java_Reading_Txtfile_Text_01\\combine\\More1";
		...
		{
			BufferedReader in = new BufferedReader(new FileReader(list.get(k))); // 입력 스트림(FileReader 객체)를 // 설마 한 번만 읽고 끝나나? 넵
			...
			{// 라인 없을 때까지 읽음 // 라인의 size 측정 { // 한 줄 단위로 처리할 수 있는 readLine 메소드를 이용해서
				...
				if (s.contains("1)") && (s.contains("회사") || s.contains("부서")) && User_Divider.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)")) 
				{
				...
				if(s.contains("보낸 사람") && !s.contains("이문영") && !s.contains("김원영") && s.contains("@")) { // 없으면 안 담음 // 한줄짜리는 여기서 다 담김
					...
					Email_Address_Last_Idx = AllText.indexOf(">");
					Email_Address = AllText.substring(0, Email_Address_Last_Idx); // 끝 인덱스는 포함 X 이고, allText 정제 ( > 기준으로 짤림 )
					Email_Address_First_Idx = Email_Address.indexOf("<");
				...
				if( s.contains("1)") && (s.contains("부서:") || s.contains("부서 :")) && Institution_Department != "소속부서 없음(개인사용자)" )
				{
					
					String subText_Institution_Department = AllText.substring(Institution_Department_First_Idx).trim();
					
					if(subText_Institution_Department.contains("지역") && ( subText_Institution_Department.contains("2)") || subText_Institution_Department.contains("2.") )) {
						...
						Institution_Department 
						= AllText.substring(Institution_Department_First_Idx + 3, Institution_Department_First_Idx+Institution_Department_Last_Idx).trim();
				...
				else if( (s.contains("2)") || s.contains("2.")) && (s.contains("부서:") || s.contains("부서 :")) && Institution_Department != "소속부서 없음(개인사용자)" )
				{
					String subText_Institution_Department_2 = AllText.substring(Institution_Department_First_Idx).trim();
					
//					if (Institution_Department == "없음") {
					if( (subText_Institution_Department_2.contains("3)") || subText_Institution_Department_2.contains("3.")) ) {
...
//					}
				...
				// 부서가 1번에 위치해 있을 때 - 2번 전에서 끊음
				if( s.contains("1)") && (s.contains("학과:") || s.contains("학과 :")) && Institution_Department != "소속부서 없음(개인사용자)")
				{
//					Sysout
					String subText_Institution_Department_3 = AllText.substring(Institution_Department_First_Idx).trim();
					
					if(subText_Institution_Department_3.contains("지역") && ( subText_Institution_Department_3.contains("2)") || subText_Institution_Department_3.contains("2.") )) {
						Institution_Department_Last_Idx = subText_Institution_Department_3.indexOf("2)");
						
						System.out.println("2)의 위치 탐색" + Institution_Department_Last_Idx);
						
						Institution_Department 
						= AllText.substring(Institution_Department_First_Idx + 4, Institution_Department_First_Idx+Institution_Department_Last_Idx).trim();
						System.out.println("ID_1!!!!!!!!!!!!!!!!!!!!!! : " + Institution_Department);
...
//						= AllText.substring(Homepage_Address_FirstIndex, Homepage_Address_FirstIndex + Homepage_Address_LastIndex).trim();
//						}
//					
//					else {
...
//
//					Institution_Department_First_Idx = s.indexOf("학과");
//					Institution_Department_Value = s.split("학과"); // 관리자가 입력하는 정보
//					//					Institution_Department_Value = s.split("학과 :");
//					//					Institution_Department_Value = s.split("학과  :");
//
//					if (Institution_Department.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)") && Institution_Department_Value.length == 2) {
//
//						Institution_Department = s.substring(Institution_Department_First_Idx+4).trim(); // 첫번째 저장된 값
...
//				
				...
				//Institution_Location_OR_UserResidence
				 ...
				 if( (s.contains("지역:") || s.contains("지역 :") ) && Institution_Location_OR_UserResidence.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)") )
	        	 {
	        		...
	        		//확인요 Institution_Location_OR_UserResidence = Institution_Location_OR_UserResidence.trim();
	        	 }
				
				 // 기관위치 또는 거주지역 파싱 예외사항
	        	 else if(s.contains("시")&&  Institution_Location_OR_UserResidence.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)")) // 시 구까지만 출력 (나머지 데이터는 버림)
	        	 {
...
//					// 발급 목적 파싱 예외사항
//	        	 else if((s.contains("공부") || s.contains("제어")) && Key_Issuance_Purpose.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))
//	        	 {
...
//	        	 }
				 ...
				 if((s.contains("업무:") || s.contains("업무 :") || s.contains("연구:")) && Duty_OR_Research.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))
	        	 {
...
//	        		 else addLineIndex_Duty = count + 1; // 이슈 2
	        		 ...
	        		 // 위에 데이터가 공백인 경우 없음으로 처리 -> 두번째 사용자 양식에서 읽게 만들기
					  if(Duty_OR_Research.equals("")) Duty_OR_Research = "없음"; // 확인
//					  else addLineIndex_Duty = count + 1; // 없음 X ( 데이터 못 잡는 경우 )
					  addLineIndex_Duty = count + 1; // 밑에서 걸리니까 else에 넣을 필요 없음
	        	 }
				 
	        	// 담당 업무 파싱 예외사항
	        	 else if(!s.contains("부서") && (s.contains("개발")) && Duty_OR_Research.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)") && s.contains(")")) // 예외 사항 모두 묶음 (개발이 많은 사항 발생)
	        	 {
...
//        	// 엔터가 되어있는 경우 처리 ( : 밑에 데이터 있는 경우)
//				
//				  else if(addLineIndex_Duty == count && !(s.contains(":"))) 
//				  { Duty_OR_Research += s; // 문자열 붙이기
//				  
//				  if(Duty_OR_Research.contains("(예")) Duty_OR_Research =
//				  Duty_OR_Research.substring(0, Duty_OR_Research.indexOf("(예")).trim(); }
				...
				//Key_Issuance_Purpose
				 ...
				 if((s.contains("목적:") || s.contains("목적 :")) && Key_Issuance_Purpose.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))
	        	 {
...
//					
//					// 다음 줄
//	        		 if(Key_Issuance_Purpose.contains("없음"))
//	        			 Key_Issuance_Purpose = s.trim();
//	        		 else
//	        			 Key_Issuance_Purpose += s.trim(); // 문자열 붙이기 *질문! => 예)가 다음에 나와있는 상황?
//	        		 
//	        		 if(Key_Issuance_Purpose.contains("(예"))
//	        			 Key_Issuance_Purpose = Key_Issuance_Purpose.substring(0, Key_Issuance_Purpose.indexOf("(예")).trim();
//				 }
				...
				//바로 위에 코드 수정한 것
					 ...
					 //다음 즐
					 if(Key_Issuance_Purpose.contains("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))  Key_Issuance_Purpose = s;
					 else Key_Issuance_Purpose += s; 
				 }
				 
				 
	        	// 발급 목적 파싱 예외사항
	        	 if((s.contains("공부") || s.contains("제어")) && Key_Issuance_Purpose.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))
	        	 {
	        		 Key_Issuance_Purpose_First_Idx = s.indexOf(")");
	        		 Key_Issuance_Purpose = s.substring(Key_Issuance_Purpose_First_Idx + 1).trim();
				...
				//Service_Access_Path 
	        	// 검색 경로 파싱
	        	 if((s.contains("경로") || s.contains("경로:") || s.contains("경로 :")) && Service_Access_Path.equals("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))
	        	 {
	        	 ...
	        	 //Enter 처리 (
					 ...
					 //다음 즐
					 if(Service_Access_Path.contains("관리자가 입력 필요(형식 미준수 또는 파일 깨짐)"))  Service_Access_Path = s;
					 else Service_Access_Path  += s; 
				 }
			...
			System.out.println(User_Divider + " :" + User_Name +" :" + Email_Address +" :" + Institution +" :" + Institution_Department +" :" + Institution_Location_OR_UserResidence);
			System.out.println(Duty_OR_Research +" :" + Key_Issuance_Purpose +" :" + Service_Access_Path +" :" + Institution_Information_Homepage_Address);
//			
//			
//			if (Duty_OR_Research == "") {
//				
...
//			}
			...
			if (file.isFile() && file.canWrite()) {
				...
				bufferedWriter.write("\"" + User_Divider + "\"" + "\t" + "\"" + User_Name + "\"" + "\t" + "\"" + Email_Address + "\"" + "\t" 
						+ "\"" + Institution + "\"" + "\t" + "\"" + Institution_Department + "\"" + "\t" + "\"" + Institution_Location_OR_UserResidence 
						+ "\"" + "\t" + "\"" + Duty_OR_Research + "\"" + "\t" + "\"" + Key_Issuance_Purpose + "\"" + "\t" + "\"" + Service_Access_Path 
						+ "\"" + "\t" + "\"" + Institution_Information_Homepage_Address + "\"" + "\t");
				bufferedWriter.newLine();
